...
cleaned_dataset = dataset.map(
    clean_batch, batched=True, batch_size=1000, num_proc=num_cores, desc="Cleaning text"
)
...
max_lengths_non_zeros = [
    item for item in length_dataset["train"]["lengths"] if item not in [0]
]
...
tokenized_dataset_text = lm_dataset.filter(
...
class TransformerModel(nn.Module):
    ...
    def __init__(
        self, vocab_size, embed_dim, num_heads, num_layers, max_seq_len, dropout_prob
    ):
        super().__init__()
...
for iter in range(max_iters):
    ...
    if iter % eval_interval == 0 or iter == max_iters - 1:
        loss_dict = estimate_loss(
            model, eval_iters=eval_iters, batch_size=batch_size, splits=("train", "val")
        )
        print(
            f"iter {iter}: train loss {loss_dict['train']:.4f}, val loss {loss_dict['val']:.4f}"
        )
